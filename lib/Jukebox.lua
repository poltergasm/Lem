...
function Jukebox:play() self.songs[self.current].source:play() end
function Jukebox:pause() self.songs[self.current].source:pause() end
function Jukebox:stop() self.songs[self.current].source:stop() end
...
function Jukebox:add_song(s)
  if s.file ~= nil then
    ...
    self.songs[#self.songs + 1] = {
      source = love.audio.newSource(s.file, self.stream or true),
      name   = s.name or nil
    }
  end
end
...
function Jukebox:update(dt)
  ...
  if #self.songs > 0 then
    ...
    if not song:isPlaying() and not song:isPaused() then
